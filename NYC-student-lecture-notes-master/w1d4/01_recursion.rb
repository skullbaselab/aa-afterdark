...
def permutations!(array) # '!' methods modify argument values
  ...
  perms.each do |perm|
    (0..perm.length).each do |i|
      # '+' operator on array returns new array object
...
def bsearch(nums, target)
  ...
  when 1
    ...
    # if target not found, don't increment sub_answer (throws error)
