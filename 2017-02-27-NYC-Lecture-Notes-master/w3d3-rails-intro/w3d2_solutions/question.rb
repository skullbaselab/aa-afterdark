...
class Question < ModelBase
  def self.find(id)
    sql = <<-SQL
      ...
      WHERE
        questions.id = :id
    SQL
    #here_docs are basically just double-quoted strings

    question_data = QuestionsDatabase.get_first_row(sql, id: id)
    #gets the first row found in query (just the hash)
  ...
  def self.find_by_author_id(author_id)
    ...
    #both of these ways sanitize!
    #directly interpolating into here_docs does not.
    #cannot use 'binding params' in FROM clause
