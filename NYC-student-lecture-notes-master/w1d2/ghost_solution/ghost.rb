...
class GhostGame
  ...
  def display_standings
    return if losses.all? { |_, v| v == 0 } # block takes key, value pair, but we don't need key, hence '_'
    ...
    sleep(2) # tells ruby to wait 2 seconds
  end
  ...
  def remaining_players
    losses.count { |_, v| v < MAX_LOSS_COUNT } # again, don't need the key in this case
  end
  ...
  def valid_play?(letter)
    return false unless ALPHABET.include?(letter) # ALPHABET is constant (cannot be modified)
  ...
  def winner
    # (player, _) destructure return array
    (player, _) = losses.find { |_, losses| losses < MAX_LOSS_COUNT } # again, don't need key
    player
  end
end
