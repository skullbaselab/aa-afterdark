...
class Array
  def dups
    positions = Hash.new { |hash, key| hash[key] = [] }
      # assign new empty array every time       ^^^
...
class Array
  def merge_sort(&prc)
    ...
    return self if count <= 1 # empty and single-element arrays are sorted
  ...
  def self.merge(left, right, &prc)
    ...
    merged.concat(left)
    merged.concat(right)
    # we concat here because one of the arrays may not be empty
    # the non-empty array will contain all of the largest elements, in order,
    # so concat will clean up nicely for us
