require "set" # gotta pull set from ruby, a gem that we installed
require_relative "player" # look for a fil local to our project
...
class GhostGame
  ...
  # Whats up with _? Allowing good code readability signifying
  # that the key is not used in this block.
  # _ is now a variable that houses the key (we just aint gonna use it)
  def remaining_players
    losses.count { |_, v| v < MAX_LOSS_COUNT }
  end

  def winner
    # losses.find returns a tuple. Leveraging parallel assignment
